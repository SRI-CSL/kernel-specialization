...
def format_symbol_line(line):
	...
	if len(line) == 1: # symbol name not present
		line.append("")
	return (line[0], line[1])

def get_ks_path():
	return os.environ["KS_PATH"]
...
def get_native_lib_UW_symbols(native_libs):
	...
	for path in native_libs:
		...
		for nlb in files:
			if os.path.basename(nlb) == "libgcc.a":
				continue
...
def usedInAsm(modules):
	symbols = []
	for module in modules:
		print module
		so_fname = get_ks_path() + "/LLVMPasses/build/UsedInAsm.so"
		cmd = "opt -load %s -used-in-asm %s -o tmp.bc" % (so_fname, module)
		p = subprocess.Popen(cmd.split(' '), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
		symbols += p.communicate()[1].split('\n')
	return symbols
